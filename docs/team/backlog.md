...
## Sound  — *trimmed to two items by PO decision; tech-led by the sound-engineer agent*

> Added 2026-06-22 as an **unrequested** research/definition cascade from the
> sound-engineer role; **trimmed 2026-06-23 by PO decision.** The original 5-epic
> track was scope nobody asked for (the focus was mechanics + graphics), so all of it
> was closed **except** the two items that earn their keep on user-value grounds: an
> existing-feature correctness bug (S4) and a tiny payoff for the just-shipped M1 arc
> (the S2 completion sting). **Both rank BELOW the entire graphics (G*) track** — they
> never auto-pull ahead of requested graphics work. Hard constraints carry over: stay
> procedural / **0 asset bytes**, DI / no-singleton / headless-testable, click-free
> ramps, no per-frame allocation, never autoplay before a gesture, audio never the sole
> channel for critical info. Closed as out-of-scope (2026-06-23): epics S1 #86, S3 #88,
> S5 #90; stories #92–#96, #99, #100, #101–#104; ux policy question #91; and the
> superseded roadmap PR #108.

### S4 — Mobile-Safari Survival: silent-switch & interrupt recovery  *(correctness — #89)*
...
- **Goal / value:** On the target phone, opted-in sound dies silently three ways: the
  ...
  resumes the loop but not the context. This is a real bug in already-shipped behaviour
  — kept on correctness grounds regardless of the broader track being out of scope.
...
- **Slices:** #105 persistent resume net · #106 silent-element unlock · #107 consume
  context state. Independent of everything else. On-device behaviour is **"needs
  verification"** (iOS-version-sensitive).

### S2 (sting only) — Completion sting for the M1 arc  *(payoff — #87, slices #97/#98)*

- **Goal / value:** Reaching 13/13 (M1, shipped) currently replays the ordinary chime,
  underselling the one big moment we just built. Add a procedural completion sting on
  the `discovery.completed` rising edge — 0 asset bytes, mount-safe, no cross-role dep.
- **Scope:** **only** slices #97 (sting synth) and #98 (fire on rising edge). The wider
  S2 UI/guess cues (#99/#100) were closed as out-of-scope.

**Sound ranking:** strictly **below all graphics epics**. Pull order for a no-argument
run: G1 → G2 → G3 → G4, then S4 (#89), then the S2 sting (#87 / #97 / #98). S4 may be
promoted ahead of G3/G4 **only** if the user reports audio breakage on device.

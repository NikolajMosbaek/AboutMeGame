...
const scopeDirective = feature
  ? `The requested feature is the EXACT, NON-NEGOTIABLE scope of this run: "${feature}". You MUST scope the problem statement to precisely this feature. Do NOT substitute a different feature, do NOT pull a different or "more valuable" item, do NOT re-prioritize, and do NOT expand scope into process/harness/role changes. If the feature names a GitHub issue, that issue defines the work. Read docs/team/charter.md only for grounding (stack, conventions, prioritisation policy).`
  : `No explicit feature was given — pull the top \`Todo\` item from the GitHub Project board (\`gh project item-list 2 --owner NikolajMosbaek --format json\`; the top item is the highest priority — if it is an epic, take its first not-\`Done\` slice), or propose the most valuable next item if the board has no \`Todo\` items.`

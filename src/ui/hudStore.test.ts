...
describe("hudStore", () => {
  it("starts at rest, facing north, not sprinting", () => {
    ...
    expect(store.getSnapshot()).toEqual({ sprinting: false, heading: 0, compass: "N" });
  });

  it("returns a stable snapshot reference while nothing changes", () => {
    const store = createHudStore();
    const first = store.getSnapshot();
    store.set({ sprinting: false, heading: 0 });
    expect(store.getSnapshot()).toBe(first);
  });

  it("rounds heading to integers and only emits on a real change", () => {
    const store = createHudStore();
    const listener = vi.fn();
    store.subscribe(listener);

    // 12.4 and 12.6 round to 12 and 13 respectively.
    store.set({ sprinting: false, heading: 12.4 });
    expect(store.getSnapshot().heading).toBe(12);
    expect(listener).toHaveBeenCalledTimes(1);

    // Same rounded value → no new snapshot, no emit (throttles per-frame churn).
    const before = store.getSnapshot();
    store.set({ sprinting: false, heading: 12.49 });
    expect(store.getSnapshot()).toBe(before);
    expect(listener).toHaveBeenCalledTimes(1);

    // A rounded change emits once.
    store.set({ sprinting: false, heading: 12.6 });
    expect(store.getSnapshot().heading).toBe(13);
  ...
  it("emits when sprint flips, and normalises heading into 0..359", () => {
    ...
    const listener = vi.fn();
    store.subscribe(listener);
    store.set({ sprinting: true, heading: -90 });
    expect(store.getSnapshot()).toEqual({ sprinting: true, heading: 270, compass: "W" });
    expect(listener).toHaveBeenCalledOnce();
    // 450 normalises to the same 90…
    store.set({ sprinting: true, heading: 450 });
  ...
  it("stops notifying after unsubscribe", () => {
    ...
    unsub();
    store.set({ sprinting: true, heading: 10 });

 ...
 *  • top-left the cardinal compass point — the sole diegetic wayfinding cue now
 *    that GPS markers and the vehicle-era speed readout are gone — kept visible
 *    while sprinting (a small SPRINT tag rides alongside it, rather than
 *    replacing the heading exactly when you're covering ground),
    ...
    <>
      <div className="hud-telemetry" role="status" aria-label="explorer status">
        <span className={`hud-mode${h.sprinting ? " hud-mode--sprint" : ""}`}>
          <span className="hud-mode__compass">{h.compass}</span>
          {h.sprinting && <span className="hud-mode__badge">SPRINT</span>}
        </span>
      </div>

 ...
 * card that bloated from a few tens of KB to, say, 2 MB (an accidental
 * photographic re-export, a lost --scale, a PNG saved uncompressed) would still
 * leave the total comfortably under 6 MB — the current dist baseline is ~752 KB,
 * so the total cap has multi-MB of headroom and can NEVER trip on a per-image
 * regression at this scale. The total cap therefore cannot protect the per-image
 * target; only an explicit upper bound on this one file can. This guard is that
 * bound: a bloated re-export of public/social-preview.png fails HERE, loudly and
 * independently, long before it would ever move the total-payload needle.
 *
...
describe("public/social-preview.png — per-image byte bound", () => {
  ...
  it("stays at or under the documented tens-of-KB ceiling, independent of the 6 MB total cap", () => {
    const { size } = statSync(pngPath);

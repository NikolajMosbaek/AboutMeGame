...
describe("settingsStore", () => {
  ...
  it("persists and round-trips changes", () => {
    ...
    expect(store.getSnapshot()).toEqual({ muted: true, quality: "low", reducedMotion: false, showDiscoveredMarkers: false });

    // A fresh store reading the same storage sees the persisted values.
    const reloaded = createSettingsStore(storage);
    expect(reloaded.getSnapshot()).toEqual({ muted: true, quality: "low", reducedMotion: false, showDiscoveredMarkers: false });
  });
  ...
  it("loads a persisted showDiscoveredMarkers:true and round-trips set across a fresh store", () => {
    const storage = mem({
      "aboutmegame.settings.v1": JSON.stringify({ showDiscoveredMarkers: true }),
    });
    // A persisted true survives load (the validating branch, not the default).
    expect(createSettingsStore(storage).getSnapshot().showDiscoveredMarkers).toBe(true);

    // A malformed/absent value falls back to the false default.
    expect(createSettingsStore(mem()).getSnapshot().showDiscoveredMarkers).toBe(false);
    expect(
      createSettingsStore(
        mem({ "aboutmegame.settings.v1": JSON.stringify({ showDiscoveredMarkers: "yes" }) }),
      ).getSnapshot().showDiscoveredMarkers,
    ).toBe(false);

    // set persists and a fresh store reads it back true (reload reflection).
    const fresh = mem();
    createSettingsStore(fresh).set({ showDiscoveredMarkers: true });
    expect(createSettingsStore(fresh).getSnapshot().showDiscoveredMarkers).toBe(true);
  });
